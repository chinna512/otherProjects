...
{
    CGRect frame = CGRectMake(point.x, point.y, 60, 10);
    descriptionLabel.frame = frame;
    descriptionLabel.font = _descriptionTextFont;
    descriptionLabel.textColor = _descriptionTextColor;
    descriptionLabel.shadowColor = _descriptionTextShadowColor;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (_showLabel) {
        ...
        for (int i = 0; i < _axisY_labels.count; i++) {
            ...
            [self showYLabel:lb InPosition:CGPointMake(0, temp - 5)];
...
- (CAShapeLayer*) drawingPointsForChartData : (PNScatterChartData *) chartData AndWithX : (CGFloat) X AndWithY : (CGFloat) Y
{
    if (chartData.inflexionPointStyle == PNScatterChartPointStyleCircle) {
        float radius = chartData.size + 2;
        CAShapeLayer *circle = [CAShapeLayer layer];
        CGRect point1 = CGRectMake(X -radius, Y - radius, 2.0*radius, 2.0*radius);
            // Make a circular shape
        circle.path = [UIBezierPath bezierPathWithRoundedRect:point1
        ...
        circle.lineWidth = 1;
        [_pathPoints addObject:[NSValue valueWithCGRect:point1]];
        ...
        CGRect point1 = CGRectMake(X - (side/2) , Y - (side/2), side, side);
        [_pathPoints addObject:[NSValue valueWithCGRect:point1]];
        
            // Make a circular shape
        square.path = [UIBezierPath bezierPathWithRect:point1].CGPath ;
...
- (void)touchPoint:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    for (int i = 0 ; i<  _pathPoints.count ; i++){
        if (CGRectContainsPoint([[_pathPoints objectAtIndex:i] CGRectValue],touchPoint)){
            [self.delegate userClickedOnLineKeyPoint:touchPoint lineIndex:i pointIndex:i];
            isClicked = YES;
            break;
        }
    }
